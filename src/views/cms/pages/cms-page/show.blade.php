	...
	<div class="card p-4 mx-2 mx-sm-5">
		...
		@foreach ($page_fields as $field)
			...
			@elseif ($field['form_field'] == 'image')
				@include('cms::cms//components/show-fields/image', ['label' => ucwords(str_replace('_', ' ', $field['name'])), 'image' => $row[$field['name']] ])
			@elseif ($field['form_field'] == 'file')
				@include('cms::cms//components/show-fields/file', ['label' => ucwords(str_replace('_', ' ', $field['name'])), 'file' => $row[$field['name']] ])
			@elseif ($field['form_field'] == 'files')
				@include('cms::cms//components/show-fields/files', ['label' => ucwords(str_replace('_', ' ', $field['name'])), 'files' => $row[$field['name']] ])
			@elseif ($field['form_field'] == 'select')
				@include('cms::cms//components/show-fields/text', ['label' => ucwords(str_replace(['_id', '_'], ['', ' '], $field['name'])), 'text' => $row[str_replace('_id', '', $field['name'])][$field['form_field_additionals_2']] ])
			@elseif ($field['form_field'] == 'select multiple')
				@include('cms::cms//components/show-fields/text-multiple', ['label' => ucwords(str_replace(['_id', '_'], ['', ' '], $field['name'])), 'texts' => $row[$field['name']], 'display_column' => $field['form_field_additionals_2'] ])
			@elseif ($field['form_field'] == 'checkbox')
				@include('cms::cms//components/show-fields/boolean', ['label' => ucwords(str_replace('_', ' ', $field['name'])), 'value' => $row[$field['name']] ])
			@elseif ($field['form_field'] == 'map coordinates')
				@include('cms::cms//components/show-fields/map', ['label' => ucwords(str_replace('_', ' ', $field['name'])), 'name' => $field['name'], 'value' => $row[$field['name']] ])
			@else
				@include('cms::cms//components/show-fields/text', ['label' => ucwords(str_replace('_', ' ', $field['name'])), 'text' => $row[$field['name']] ])
			@endif
		@endforeach
