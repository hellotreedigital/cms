	...
	<div class="card py-4 mx-2 mx-lg-5">
		<div class="actions">
			@if (request()->get('admin')['cms_pages']['admin-roles']['permissions']['add'])
				<a href="{{ url(config('hellotree.cms_route_prefix') . '/admin-roles/create') }}" class="btn btn-primary btn-sm">Add</a>
			@endif
			@if (request()->get('admin')['cms_pages']['admin-roles']['permissions']['delete'])
				<form method="post" action="{{ url(config('hellotree.cms_route_prefix') . '/admin-roles/') }}" class="d-block d-md-inline-block bulk-delete" onsubmit="return confirm('Are you sure?')">
					@csrf
						...
						<tr>
							...
							<td class="actions-wrapper text-right">
								...
								@if (request()->get('admin')['cms_pages']['admin-roles']['permissions']['edit'])
									<a href="{{ url(config('hellotree.cms_route_prefix') . '/admin-roles/' . $row['id'] . '/edit') }}" class="mb-2 btn btn-primary btn-sm">Edit</a>
								@endif
								@if (request()->get('admin')['cms_pages']['admin-roles']['permissions']['delete'])
